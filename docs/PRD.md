...
| **F002B** | 대시보드       | 주최자가 자신의 모임들을 한눈에 관리하는 대시보드. 통계(전체/진행중/참여자), 최근 공지사항 표시                              | 대시보드 페이지 (`/protected/dashboard`)              | ✅   |
| **F002C** | 모임 수정      | 주최자가 기존 모임의 제목, 설명, 날짜, 장소, 인원, 카테고리, 마감 여부 수정. 드롭다운 메뉴의 '모임 수정'에서 접근            | 모임 수정 페이지 (`/protected/events/[eventId]/edit`) | ✅   |
| **F002D** | 모임 액션 메뉴 | 모임 카드 우측에 드롭다운 메뉴로 수정, 공지 발송, 삭제 액션 제공. 삭제 시 확인 다이얼로그 표시                               | 모임 카드 컴포넌트                                    | ✅   |
| **F003**  | 모임 홈        | 핀 고정 공지, 기본 정보(날짜, 장소, 인원 현황), 초대 링크 복사 버튼 제공                                                     | 모임 홈 페이지                                        | ✅   |
| **F004**  | 초대 링크 입장 | 비인증 사용자도 초대 링크 접근 가능. 모임 정보 미리보기, 호스트 정보 표시. 로그인 후 자동으로 참여 신청 처리 (approved 상태) | 초대 링크 처리 페이지 (`/join/[inviteCode]`)          | ✅   |
...
[초대 링크 처리 페이지 /join/[inviteCode]]
  - 비인증 사용자: 모임 정보 카드
    * 모임 이미지 배너
    * 제목, 설명, 참여자 수
    * 호스트 정보 (아바타, 이름, 호스트 배지)
    * 날짜, 장소 (아이콘 함께 표시)
    * "참여하기" 또는 "로그인하고 참여하기" 버튼
    * 마감된 모임일 경우 버튼 비활성화
  |
  | 미로그인 상태: "로그인하고 참여하기" 클릭
  v
[로그인 페이지] - 로그인 후 원래 초대 링크로 자동 복귀 (redirect=/join/[inviteCode])
  |
  | 로그인 성공 → 초대코드로 참여 신청 자동 처리 (approved 상태)
  v
[모임 홈 페이지] - 참여 신청 완료 및 자동 승인
  - 핀 공지 확인
  - 공지 목록 조회
  - 참여자 목록 확인
...
└── join/
    └── [inviteCode]/
        ├── page.tsx                         # 초대 링크 처리 (비인증 허용)
        ├── actions.ts                       # Server Action (참여하기)
        ├── join-button.tsx                  # 참여 버튼 컴포넌트
        └── not-found.tsx                    # 유효하지 않은 초대코드
```
